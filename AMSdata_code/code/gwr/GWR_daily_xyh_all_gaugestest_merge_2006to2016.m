% clc;
% clear;
% if exist('mergepre_test_2006to2016','dir')>=1
%     rmdir('mergepre_test_2006to2016','s')
% end
mkdir('mergepre_test_2006to2016');
mkdir('mergepre_test_2006to2016//ARG');
mkdir('mergepre_test_2006to2016//PRE');
%% 两阶段日降水量GWR插值程序(利用经纬度+高程信息)
%% 读取站点降雨数据和站点解释变量（地理信息）
% 1.0 读取1km网格高程，建立1km插值格网
file = dir('D://workststion//matlab//20190612GWR//input_files2006-2016//rec_mswep//txt//*.txt');
filenum = length(file);
for idx = 1293: filenum %400-20080802，827-20110701,
%【431-20080902;775-20110510;901-20110913;1201-20130907;1462-20150724】
    tempFileName = file(idx).name;
    idx 
    % 提取字符串中的数字
    str_tmp=tempFileName;
    str_digital=str_tmp(isstrprop(str_tmp,'digit'));
    pthFileName1  = sprintf('D://workststion//matlab//20190612GWR//input_files2006-2016//rec_mswep//txt//%s.tif.txt',str_digital);
    ...
    gauge_file='D:\workststion\matlab\20190612GWR\input_files2006-2016\太湖流域雨量站_all.xlsx';
    ...
    rain_file='D:\workststion\matlab\20190612GWR\input_files2006-2016\2006_2016_error.xlsx';
    ...
    %% 输出建模结果 
    target_path='D:\workststion\matlab\20190612GWR\mergepre_test_2006to2016\';
...
%     xlswrite(file_name,EE','Sheet1',index_write_result2);
    ...
    for kk=1:size(PRE,1)
        ...
        path_pre='D:\workststion\matlab\20190612GWR\mergepre_test_2006to2016\PRE\';
        path_par='D:\workststion\matlab\20190612GWR\mergepre_test_2006to2016\ARG\';
