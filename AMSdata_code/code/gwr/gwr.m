...
%                   = 'bi_square'     for tri-cube weighting
...
%% do GWR using bdwt as bandwidth/or optimal q-value from cross-validation
[n k] = size(x);
bsave = zeros(n,k);
ssave = zeros(n,k);
sigv  = zeros(n,1);
...
rsqr1 = sigu;
rsqr2 = ym'*ym;
result.rsqr = 1.0 - rsqr1/rsqr2;    % 模拟结果效率系数
rsqr1 = rsqr1/(nobs-nvar);
rsqr2 = rsqr2/(nobs-1.0);
result.rbar = 1 - (rsqr1/rsqr2);    % 校正后的模拟结果效率系数