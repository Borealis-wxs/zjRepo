...
clc;%符号
clear;
if exist('result_output_mswep_month_th','dir')>=1
    rmdir('result_output_mswep_month_th','s')
end
mkdir('result_output_mswep_month_th');
str_output=sprintf('result_output_mswep_month_th//result_mswep_month_th.txt');
file1 = dir('input//th_mswep_month//regions//th//*.txt');
file2 = dir('input//th_idw_month//regions//th//*.txt');
...
%     tempFileName2=sprintf('th_%s_idw.txt',str_digital);;
    pthFileName1  = sprintf('input//th_mswep_month//regions//th//%s',tempFileName);
    pthFileName2  = sprintf('input//th_idw_month//regions//th//%s',tempFileName2);
%     cell_1 = func_importfile_txt_blank_6_7(pthFileName1, 7, 13);
%     cell_2 = func_importfile_txt_blank_6_7(pthFileName2, 7, 13);
    cell_1 = func_importfile_txt_R21C29(pthFileName1, 7, 27);
    cell_2 = func_importfile_txt_R21C29(pthFileName2, 7, 27);
    img1 = cell2mat(cell_1);
    img2 = cell2mat(cell_2);
...
%     division=2; %除数，将降水数据都除以这个division，然后转化为（0,255）之间的图像像素数据   daypre
